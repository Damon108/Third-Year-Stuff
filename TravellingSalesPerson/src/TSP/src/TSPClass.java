...
public class TSPClass {
			...
			for (int i = 0; i < weights.length; i++) {
				...
				for (int j = 0; j < weights[i].length; j++) {
						...
						for (int counter = 0; counter < weights.length; counter++) {
							if (city[c].equals(weights[counter][0])) {
								...
								if (city[0].equals(weights[counter][1])) {
									 ...
									 * else if
									 * (city[0].equals(weights[counter][1])) {
					...
					if (city[c].equals(weights[i][0])) {
						...
						/*
						 * if (c == 3) { System.out.println("final total: " +
						 * sCost); }
						 */
	...
	public String generateRandomRoutes() {
		...
		for (float[] route : cities) {
			...
			// Store all 16 cities in temp Array
			troute[counter] = route[0];

			System.out.println("Cities inside troute : " + troute[counter]);
			Sroute += String.valueOf(route[0]);
		...
		boolean AChosen = false;
		boolean BChosen = false;
		boolean CChosen = false;
		...
		for (int i = 0; i < 100; i++) {
			...
			if (num == 0 && AChosen == false) {
				AChosen = true;
				route += "A";
			} else {
				if (num == 1 && BChosen == false) {
					BChosen = true;
					route += "B";
				} else {
					if (num == 2 && CChosen == false) {
						CChosen = true;
						route += "C";
					} else {
						if (num == 3 && DChosen == false) {
							DChosen = true;
							route += "D";
						}
					}
		...
		System.out.println(DChosen);
		System.out.println("Random Route : " + route);
		return route;
	}
